class ExtImage {
    ...
    setPixelRGBA(x: number, y: number, color: number, alpha: number = 1) {
        ...
        if (this.hasAlpha) this.channel['alpha'][~~x % this._width + ~~y % this._height * this._width] = Math.min(alpha, 1);
    ...
    setPixelComponents(x: number, y: number, {r, g, b, a}, blend: string = '') {
        if (x < 0 || y < 0 || x >= this._width || y >= this._height) return;

        let pos = ~~x % this._width + ~~y % this._height * this._width;

        if (blend === 'over') {
            let fr = (r * a) + (this.channel['red'][pos] * (1.0 - a));
            let fg = (g * a) + (this.channel['green'][pos] * (1.0 - a));
            let fb = (b * a) + (this.channel['blue'][pos] * (1.0 - a));
            // let fa = (a * a) + (this.channel['alpha'][pos] * (1.0 - a));

            this.channel['red'][pos] = Math.min(fr, 1);
            this.channel['green'][pos] = Math.min(fg, 1);
            this.channel['blue'][pos] = Math.min(fb, 1);
            this.channel['alpha'][pos] = Math.min(this.channel['alpha'][pos] + a, 1);
        } else {
            this.channel['red'][pos] = Math.min(r, 1);
            this.channel['green'][pos] = Math.min(g, 1);
            this.channel['blue'][pos] = Math.min(b, 1);
            if (this.hasAlpha) this.channel['alpha'][pos] = Math.min(a, 1);
        }
