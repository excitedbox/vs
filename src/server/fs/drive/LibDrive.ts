...
export default class LibDrive implements IDrive {
    ...
    async readFile() {
        if (this.path.slice(0, 4) === ".//[") {
            // Parse array list
            let finalPath = this.path.slice(3)
                .replace(/[\[\]]/g, '')
                .split(',')
                .map(x => x.trim());
            let outData = [];
            ...
            for (let i = 0; i < finalPath.length; i++)
                outData.push(await this.getLibFile(finalPath[i]));

            // Return all libs
            return outData.join('\n\n');
        } else return await this.getLibFile(this.path);
    }
    ...
    private async getLibFileFromCDN(libPath: string) {
        ...
        let libName = libPath.replace('./', '').replace(/\//g, '');
